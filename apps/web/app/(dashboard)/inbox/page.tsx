...
function ChannelBadge({ type }: { type: string }) {
  ...
  return (
    <span
      className={`inline-flex items-center justify-center w-4 h-4 rounded-sm text-white font-bold text-[9px] ${cfg.bg} shrink-0`}
    >
      {cfg.icon}
    </span>
  );
      ...
      <div className="flex items-start gap-2.5">
        <div className="relative shrink-0">
          <div className="w-8 h-8 rounded-full bg-gradient-to-br from-indigo-500 to-purple-600 flex items-center justify-center text-white text-xs font-bold">
            {(conv.customer?.name ?? conv.customerId)
              ?.charAt(0)
              ?.toUpperCase() ?? "?"}
          </div>
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="flex items-center gap-1.5 mt-1.5">
            <span
              className={`text-[10px] font-medium px-1.5 py-0.5 rounded-full ${status.color}`}
            >
              {status.label}
            </span>
          </div>
  ...
  if (type === "sticker") {
    return (
      <p className="text-2xl">
        🖼️ <span className="text-xs text-gray-500">Sticker</span>
      </p>
    );
  }
  return null;
...
function MessageBubble({ msg }: { msg: Message }) {
  ...
  return (
    ...
    >
      {isCustomer && (
        <div className="w-6 h-6 rounded-full bg-gradient-to-br from-indigo-500 to-purple-600 flex items-center justify-center text-white text-[10px] font-bold shrink-0 mt-1">
          C
        </div>
      )}
      <div
        className={`max-w-[70%] ${isCustomer ? "" : "items-end flex flex-col"}`}
      >
        {isMedia ? (
          ...
          >
            ...
            {msg.translatedContent &&
              ...
              isCustomer && (
                <p className="text-[11px] text-gray-500 mt-1.5 pt-1.5 border-t border-white/5 italic">
                  {msg.translatedContent}
                </p>
              )}
          </div>
    ...
    <div className="w-64 border-l border-white/5 flex flex-col bg-[#0d0d1a] shrink-0">
          ...
          <div className="min-w-0">
            ...
            <div className="flex items-center gap-1 mt-0.5">
              ...
              <span className="text-[11px] text-gray-500">
                {CHANNEL_CONFIG[channelType]?.label}
              </span>
            </div>
          </div>
        </div>
      ...
      <div className="flex-1 overflow-y-auto p-4">
        {tab === "info" && (
          <div className="flex flex-col gap-3">
            ...
            <div className="mt-1">
              <p className="text-[11px] text-gray-600 uppercase tracking-wide font-medium mb-2">
                Channels
              </p>
              {customer?.channelProfiles?.length ? (
                customer.channelProfiles.map((cp: any) => (
                  <div
                    key={cp.externalId}
        ...
        {tab === "orders" && (
          <div className="flex flex-col gap-3">
            <div className="flex items-center gap-2 text-xs text-gray-600">
              <ShoppingBag size={13} />
              <span>ยังไม่มี order integration</span>
            </div>
            <p className="text-[11px] text-gray-600 leading-relaxed">
              เชื่อมต่อ Shopee / Lazada / TikTok Shop เพื่อดู order ที่นี่
            </p>
          </div>
        )}
        {tab === "history" && (
      ...
      <div className="p-3 border-t border-white/5 flex flex-col gap-1.5">
        <div className="flex items-center justify-between text-xs">
          ...
          <span
            className={`px-2 py-0.5 rounded-full font-medium text-[10px] ${STATUS_CONFIG[conv?.status]?.color}`}
          >
...
export default function InboxPage() {
  const { conversations, activeId, setConversations, setActiveId } =
    useInboxStore();
    ...
    try {
      ...
      const type = file.type.startsWith("image/")
        ...
        : file.type.startsWith("video/")
          ? "video"
          : "file";
      setPendingMedia({ url: data.url, filename: file.name, type });
    } finally {
      setUploading(false);
      e.target.value = "";
    }
  ...
  async function sendMessage() {
    if ((!input.trim() && !pendingMedia) || !activeId) return;
    setSending(true);
    try {
      if (pendingMedia) {
        ...
        if (input.trim()) {
          const { data: d2 } = await api.post(
            `/api/conversations/${activeId}/messages`,
            { content: input },
          );
          setMessages((prev) => [...prev, d2.message]);
          setInput("");
        }
      } else {
        const { data } = await api.post(
        ...
        {active ? (
          <>
            ...
            <div className="px-4 py-3 border-b border-white/5 flex items-center justify-between bg-[#0d0d1a]">
              <div className="flex items-center gap-3">
                <div className="relative">
                  <div className="w-8 h-8 rounded-full bg-gradient-to-br from-indigo-500 to-purple-600 flex items-center justify-center text-white text-xs font-bold">
                    {(active.customer?.name ?? active.customerId)
                      ?.charAt(0)
                      ?.toUpperCase() ?? "?"}
                  </div>
                  <span className="absolute -bottom-0.5 -right-0.5">
                    <ChannelBadge
                      type={active.channelType ?? active.channel?.type}
                    />
                  </span>
                </div>
                <div>
                  <p className="text-sm font-semibold text-white">
                    {active.customer?.name ??
                      `User ···${active.customerId?.slice(-4)}`}
                  </p>
                  <p className="text-[11px] text-gray-500">
                    {
                      CHANNEL_CONFIG[active.channelType ?? active.channel?.type]
                        ?.label
                    }{" "}
              ...
              <div className="flex items-center gap-2">
                <button
                  type="button"
                  onClick={async () => {
                    await api.patch(`/api/conversations/${active.id}`, {
                      status:
                        active.status === "RESOLVED" ? "OPEN" : "RESOLVED",
                    });
                    refetchConversations();
                  }}
                ...
                <button
                  type="button"
                  className="p-1.5 rounded-lg text-gray-500 hover:bg-white/5 hover:text-gray-300 transition"
                >
            ...
            <div className="border-t border-white/5 bg-[#0d0d1a]">
              ...
              {/* Media preview */}
              {pendingMedia && (
                <div className="mx-3 mt-2 flex items-center gap-2 bg-white/5 border border-white/10 rounded-xl px-3 py-2">
              ...
              <div className="flex items-end gap-2 px-3 pb-3 pt-2">
                ...
                <div
                  className={`flex-1 rounded-2xl border transition-colors ${isNote ? "bg-amber-500/5 border-amber-500/20" : "bg-white/5 border-white/10 focus-within:border-indigo-500/50"}`}
                >
                  <input
                    ref={inputRef}
                    value={input}
          ...
          <div className="flex-1 flex flex-col items-center justify-center text-gray-600 gap-4">
            ...
            <div className="text-center">
              <p className="text-sm font-medium text-gray-500">
                เลือก conversation เพื่อเริ่มต้น
              </p>
